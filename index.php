...
if ($cat_filter > 0) {
    $sql = "SELECT p.*, c.category_name FROM posts p LEFT JOIN categories c ON p.category_id = c.id WHERE p.category_id = $cat_filter ORDER BY p.created_at DESC";
} else {
    $sql = "SELECT p.*, c.category_name FROM posts p LEFT JOIN categories c ON p.category_id = c.id ORDER BY p.created_at DESC";
}
